...
async function searchNews(query) {
  ...
  resultsContainer.innerHTML = '<p class="news-placeholder">🔍 Searching...</p>';
  ...
  try {
    ...
    if (!data.success || !data.results || data.results.length === 0) {
      resultsContainer.innerHTML =
        '<p class="news-placeholder">No results found. Try a different search.</p>';
      return;
    }
    ...
    resultsContainer.innerHTML =
      '<p class="news-placeholder">⚠️ Search failed. Please try again.</p>';
  }
}
// ===== INPUT HELPERS =====
